...
static void process_dns_query(const DNSServer* server, const uint8_t* query, size_t query_len,
                            uint8_t* response, size_t* response_len) {
    if (query_len < sizeof(DNSHeader)) {
        *response_len = 0;
        return;
    }
...
bool dns_server_start(DNSServer* server) {
    ...
    while (server->running) {
        ...
        ssize_t query_len = recvfrom(server->socket_fd, query, sizeof(query), 0,
